...
export function ServiceForm({ service }: Props) {
  ...
  const [fullDescription, setFullDescription] = useState(service?.fullDescription ? JSON.stringify(service.fullDescription, null, 2) : '{"type":"doc","content":[]}');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const data = { name, slug, icon, shortDescription, fullDescription: JSON.parse(fullDescription), techStack: techStack.split(",").map(s => s.trim()).filter(Boolean), order, published };
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
      ...
      <div><label className="block text-sm font-medium text-[var(--foreground)]">Full Description (Tiptap JSON)</label><textarea value={fullDescription} onChange={e => setFullDescription(e.target.value)} rows={8} className="mt-1 block w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 font-mono text-xs text-[var(--foreground)] focus:border-[var(--ring)] focus:outline-none focus:ring-1 focus:ring-[var(--ring)]" /></div>
